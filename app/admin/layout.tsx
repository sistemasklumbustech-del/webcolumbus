...
function IconoOperacion({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.75} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Mismo patrón que panel-empresa/layout.tsx -- preferencia por
  // navegador, clave propia para no compartir estado entre paneles.
  useEffect(() => {
    try {
      setMenuColapsado(localStorage.getItem("panelAdminMenuColapsado") === "1");
    } catch {
  ...
  function alternarMenuColapsado() {
    setMenuColapsado((previo) => {
      const nuevo = !previo;
      try {
        localStorage.setItem("panelAdminMenuColapsado", nuevo ? "1" : "0");
      } catch {
  ...
  return (
    <div className="flex min-h-full flex-1 bg-brand-light/20">
      ...
      >
        ...
        <div className={`flex h-16 items-center gap-2 bg-brand-cobalto ${menuColapsado ? "justify-center px-2" : "px-6"}`}>
          {menuColapsado ? (
